...
public class Card {
     ...
     */
    //カード作成とシャッフル処理のメソッド
    public void shuffleCards(boolean hasJorker) {
        //トランプリストを作成
        setTrumpList(hasJorker);
        //トランプをシャッフル
        shuffleTrumpList();
    }

    //ArrayListにトランプを用意する
    private void setTrumpList(boolean hasJorker) {
        //スートの数だけ1~13の数字を生成するのを繰り返す
        for (int suitNum = Constant.CARD_INITIAL_NUM; suitNum < Constant.TRUMP_SUIT_NUM; suitNum++) {
            //スートに対して1~13までの数字を当てはめてく
            for (int trumpNum = Constant.CARD_INITIAL_NUM; trumpNum < Constant.MAX_NUMBER_OF_TRUMP; ++trumpNum) {
                //スートによって数字を決める
